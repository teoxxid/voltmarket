...
const Register: React.FC = () => {
  ...
  // 🔹 Редирект только если авторизация УЖЕ проверена и успешна
  useEffect(() => {
    if (isAuthenticated) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (registerThunk.fulfilled.match(result)) {
      // 🔹 Небольшая задержка, чтобы Redux и localStorage успели обновиться
      ...
      // Редирект на главную (ProtectedRoute теперь пропустит, т.к. isAuthenticated=true)
      navigate('/', { replace: true });
    } else {
