...
// 🔹 MOCK-ДАННЫЕ для демонстрации на GitHub Pages (без бэкенда)
const MOCK_SERVICES: Service[] = [
  {
    ...
    brand: 'Apple',
    rating: 4.9,
    image_url: 'https://via.placeholder.com/300x300/005bff/ffffff?text=iPhone+16+Pro',
    status: 'active',
    weight: 0.22,
  },
  {
    ...
    brand: 'Samsung',
    rating: 4.8,
    image_url: 'https://via.placeholder.com/300x300/1a1a1a/ffffff?text=Galaxy+S24',
    status: 'active',
    weight: 0.23,
  },
  {
    ...
    brand: 'Apple',
    rating: 4.9,
    image_url: 'https://via.placeholder.com/300x300/cccccc/333333?text=MacBook+Pro',
    status: 'active',
    weight: 2.1,
  },
  {
    ...
    brand: 'Sony',
    rating: 4.7,
    image_url: 'https://via.placeholder.com/300x300/000000/ffffff?text=Sony+XM5',
    status: 'active',
    weight: 0.25,
  },
  {
    ...
    brand: 'Apple',
    rating: 4.6,
    image_url: 'https://via.placeholder.com/300x300/e0e0e0/333333?text=iPad+Air',
    status: 'active',
    weight: 0.46,
  },
  {
    ...
    brand: 'Apple',
    rating: 4.8,
    image_url: 'https://via.placeholder.com/300x300/ff6b6b/ffffff?text=Watch+S9',
    status: 'active',
    weight: 0.04,
  },
  {
    ...
    brand: 'Dell',
    rating: 4.7,
    image_url: 'https://via.placeholder.com/300x300/333333/ffffff?text=Dell+XPS',
    status: 'active',
    weight: 1.8,
  },
  {
    ...
    brand: 'Apple',
    rating: 4.8,
    image_url: 'https://via.placeholder.com/300x300/f5f5f5/333333?text=AirPods+Pro',
    status: 'active',
    weight: 0.05,
  },
];
...
const ServiceList: React.FC = () => {
  ...
  // 🔹 Локальные состояния для инпутов фильтров
  ...
  // 🔹 Используем мок-данные (для GitHub Pages / демо-режима)
  const allServices = MOCK_SERVICES;
  const loading = false;

  // 🔹 Фильтрация товаров (мемоизируем для производительности)
  ...
  // 🔹 Синхронизация фильтров из URL при загрузке страницы
  useEffect(() => {
    ...
    // Обновляем Redux, если фильтры в URL отличаются
    ...
    // Синхронизируем локальные инпуты с фильтрами
  ...
  const handleApplyFilters = () => {
    ...
    // Обновляем URL для сохранения состояния при навигации
  ...
  const handleAddToCart = async (e: React.MouseEvent, service: Service) => {
    ...
    try {
      // 🔹 В мок-режиме показываем уведомление без реального запроса
      if (import.meta.env.MODE === 'mock' || !import.meta.env.VITE_API_URL) {
        dispatch(
          showNotification({
            type: 'success',
            message: `«${service.name}» добавлен в заказ (демо-режим)`,
          })
        );
      } else {
        // 🔹 В реальном режиме — вызываем thunk
  ...
  return (
    <div className="container">
          ...
          {services.map((service) => (
            ...
            >
              ...
              <div className="product-info">
                ...
                {service.rating && (
                  <p className="product-rating">★ {service.rating}</p>
                )}
      ...
      <div className="catalog-footer">
        Найдено товаров: <strong>{services.length}</strong> из {allServices.length}
      </div>
    </div>
  );
};
