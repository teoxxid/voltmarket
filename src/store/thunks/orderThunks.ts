...
// Тип для параметров фильтрации заказов
    ...
    try {
      ...
      if (response.data?.status === 'success' && response.data.data) {
        ...
        if (order_id !== null && order_id !== undefined) {
          dispatch(setCart({ orderId: order_id, items: [] }));
        }
    ...
    try {
      ...
      if (response.data?.status === 'success' && response.data.data) {
        ...
        if (state.cart.orderId !== order_id && order_id) {
          dispatch(setCart({ orderId: order_id, items: state.cart.items || [] }));
        }
      ...
      try {
        ...
        dispatch(showNotification({ 
          type: 'info', 
          message: 'Товар добавлен (локально)' 
        }));
      ...
      return rejectWithValue(err.response?.data?.message || 'Ошибка добавления в корзину');
    }
  }
);
...
// 🔹 Получение списка заказов — ИСПРАВЛЕНО: параметр не опциональный, но может быть undefined
...
// 🔹 Завершение заказа (для модератора)
...
// 🔹 Отклонение заказа (для модератора)
